...
comercial_artigo <- aggregate(comercial$Qtde_Total, list(comercial$`Gamma Product Code`, comercial$Consignado), FUN = sum)
names(comercial_artigo) <- c("ArtigoGamma", "Consignado", "Qtde")

comercial_artigo <- comercial_artigo %>%
  group_by(ArtigoGamma) %>%
  summarise(Qtde = sum(Qtde), Consignado = max(Consignado))

comercial_artigo <- comercial_artigo[comercial_artigo$Qtde > 0, ]
...
aduaneiro <- aduaneiro[aduaneiro$`Fec. Entrada` < 20240101, ]
str(aduaneiro)
View(aduaneiro)
sum(aduaneiro$Dispnibl)

# consolidação artigo / empresa
aduaneiro_artigo <- aggregate(aduaneiro$`Dispnibl`, list(aduaneiro$Artículo), FUN = sum)
names(aduaneiro_artigo) <- c("ArtigoGamma", "Qtde")
...
# Comparacao
comparacao <- merge(comercial_artigo, aduaneiro_artigo, by = c("ArtigoGamma"), all = TRUE)
View(comparacao)
...
diferenca <- arrange(diferenca, desc(abs(diff)))

write_xlsx(diferenca, str_replace('diferenca_UN_DP_MESREF.csv', 'MESREF', mes_ref))
